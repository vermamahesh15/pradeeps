...
$post = $post ?? [];
$postTitle = trim($post['title'] ?? '') ?: 'झरिहख';
$postCategory = trim($post['category_name'] ?? '') ?: 'अवधी संस्मरण';
$postAuthor = trim(($post['author'] ?? '') ?: ($post['author_name'] ?? 'श्री प्रदीप सारंग'));
$postDate = !empty($post['published_at']) ? date('d M Y', strtotime($post['published_at'])) : date('d M Y');
$bannerImage = !empty($post['banner_image']) ? base_url($post['banner_image']) : base_url('assets/images/slider_final_1.webp');
